...
<div class="wrapper tw-flex tw-h-screen tw-overflow-hidden tw-bg-slate-50">
    ...
    <div id="content" class="tw-flex tw-flex-col tw-flex-1 tw-min-w-0 tw-overflow-y-auto">
        ...
        <main class="tw-flex-1 tw-p-4 sm:tw-p-6 lg:tw-p-8">
            ...
            <div class="tw-bg-primary-900 tw-rounded-2xl tw-px-7 tw-py-6 tw-mb-6 tw-shadow-blue">
                <div class="tw-flex tw-items-center tw-gap-4 tw-mb-3">
                    ...
                    <h1 class="tw-text-white tw-font-bold tw-text-xl tw-mb-0">Nouvelle Ordonnance Médicale</h1>
                </div>
            ...
            <div class="tw-bg-white tw-rounded-2xl tw-shadow-card tw-border tw-border-slate-100 tw-overflow-hidden">
                ...
                <div class="tw-px-6 tw-py-4 tw-bg-slate-50 tw-border-b tw-border-slate-200 tw-flex tw-items-center tw-justify-between">
                    <div class="tw-flex tw-items-center tw-gap-3">
                        <div class="tw-w-10 tw-h-10 tw-rounded-xl tw-bg-primary-700 tw-text-white tw-flex tw-items-center tw-justify-center tw-shadow-blue">
                            <i class="fas fa-list-ul tw-text-sm"></i>
                        </div>
                        <div>
                            <p class="tw-font-bold tw-text-slate-800 tw-mb-0 tw-text-sm">Liste des Médicaments</p>
                            <p class="tw-text-xs tw-text-slate-400 tw-mb-0">Remplissez chaque ligne avec précision</p>
                        </div>
                    </div>
                    <div class="tw-inline-flex tw-items-center tw-gap-2 tw-bg-primary-700 tw-text-white tw-text-sm tw-font-bold tw-px-3.5 tw-py-1.5 tw-rounded-full tw-font-mono">
                ...
                <div class="tw-px-6 tw-py-4 tw-bg-slate-50 tw-border-t tw-border-slate-200 tw-flex tw-flex-wrap tw-items-center tw-justify-between tw-gap-4">
                    <div class="tw-flex tw-items-center tw-gap-3">
                        <div class="tw-w-11 tw-h-11 tw-rounded-xl tw-bg-primary-700 tw-text-white tw-flex tw-items-center tw-justify-center tw-shadow-blue">
                    ...
                    <button type="submit" form="prescriptionForm"
                            class="tw-inline-flex tw-items-center tw-gap-2.5 tw-bg-primary-700 hover:tw-bg-primary-800 tw-text-white tw-font-bold tw-text-base tw-px-7 tw-py-3 tw-rounded-xl tw-shadow-blue tw-transition-all tw-duration-150 tw-border-0 tw-cursor-pointer"
                            id="submitBtn">
                        <i class="fas fa-file-medical tw-text-sm"></i>
...
$(function () {
    ...
    $.ajax({
        ...
        method: 'GET',
        success: function (data) { produitsData = data; },
        error:   function (xhr, st, err) { console.error('Erreur produits:', err); },
        complete: function () { addRow(); }
    });
    ...
    function addRow () {
        ...
        <tr id="row_${seq}" data-seq="${seq}" style="animation:rowFadeIn .35s ease both">
            <td class="tw-px-4 tw-py-3 tw-align-top tw-pt-4">
                <span class="tw-inline-flex tw-items-center tw-justify-center tw-w-6 tw-h-6 tw-rounded-full tw-bg-primary-700 tw-text-white tw-text-[11px] tw-font-bold tw-font-mono rx-row-num">${rowCount}</span>
            </td>
            ...
            <td class="tw-px-4 tw-py-3 tw-text-center">
                <div class="tw-flex tw-justify-center">${actionBtn}</div>
            </td>
        </tr>`;
        $('#rxRows').append(html);
        updateCount();
        ...
        if (rowCount > 1) $('html,body').animate({ scrollTop: $(`#row_${seq}`).offset().top - 160 }, 400);
    }
    ...
    function initAC (seq) {
        ...
        $in.on('focus', function () { const v = $(this).val().trim(); if (v.length >= 2) showSuggestions(v, $in, $drop); });
        $in.on('blur',  function () { setTimeout(() => { $drop.hide().empty(); currentFocus = -1; }, 200); });
    }
    ...
    function showSuggestions (term, $in, $drop) {
        ...
        if (!exact && term.length >= 2) {
            const $custom = $(`<div class="rx-drop-item rx-drop-custom" data-val="${esc(term)}">
                ...
                <div class="tw-text-xs tw-text-slate-500"><i class="fas fa-user-md tw-mr-1"></i>Médicament personnalisé (non répertorié)</div>
            </div>`);
    ...
    $('#prescriptionForm').on('submit', function (e) {
        ...
        $(this).find('[required]').each(function () {
            if (!$(this).val().trim()) {
                ok = false; $(this).addClass('is-invalid');
        ...
        $('#submitBtn').prop('disabled', true).html(`<span class="spinner-border spinner-border-sm me-2"></span>Génération en cours…`);
    });
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\ADAM EDJABE\Desktop\cmcuapp\resources\views/admin/prescriptions/ordonance_create.blade.php ENDPATH**/ ?>