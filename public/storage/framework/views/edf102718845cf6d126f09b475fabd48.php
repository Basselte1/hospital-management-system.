...
<?php $__env->startSection('title', 'CMCU | Modifier ordonnance'); ?>
...
<div class="wrapper">
    ...
    <div class="container-fluid py-4">
        ...
        <div class="rx-header">
            <div class="position-relative">
                <div class="d-flex align-items-center mb-3">
                    ...
                    <h1 class="h3 mb-0 fw-bold">Modifier Ordonnance Médicale</h1>
                </div>
        ...
        <div class="rx-card">
                ...
                <div class="d-flex align-items-center justify-content-between">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            ...
                            <small class="text-muted">Modifiez les lignes selon les besoins</small>
                        </div>
                    </div>
            ...
            <div class="p-4">
                <form method="POST" id="prescriptionForm" action="<?php echo e(route('ordonances.update', $ordonance->id)); ?>">
                    <?php echo csrf_field(); ?>
                    ...
                    <div class="rx-table-wrap">
                        <table class="table mb-0" id="rxTable">
                            ...
                            <tbody id="rxRows">
                                ...
                                <?php $__currentLoopData = $medicaments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $medicament): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <?php
                                        $seq = $index + 1;
                                    ?>
                                    <tr id="row_<?php echo e($seq); ?>" data-seq="<?php echo e($seq); ?>" style="--delay:<?php echo e(($index)*50); ?>ms;animation:rowFadeIn .35s ease both">
                                        <td style="padding-top:1.1rem">
                                            <span class="rx-row-num"><?php echo e($index + 1); ?></span>
                                        </td>
                                        <td>
                                            <div class="rx-ac-wrap">
                                                <div class="rx-ac-field">
                                                    <input type="text"
                                                           id="med_<?php echo e($seq); ?>"
                                                           name="medicament[]"
                                                           class="rx-input med-input"
                                                           data-seq="<?php echo e($seq); ?>"
                                                           placeholder="Ex : Doliprane 1000 mg"
                                                           autocomplete="off"
                                                           value="<?php echo e(trim($medicament)); ?>"
                                                           required />
                                                </div>
                                                <div class="rx-dropdown" id="drop_<?php echo e($seq); ?>" style="display:none"></div>
                                            </div>
                                        </td>
                                        <td>
                                            <textarea name="description[]"
                                                      ...
                                                      required><?php echo e(trim($descriptions[$index] ?? '')); ?></textarea>
                                        </td>
                                        <td>
                                            <input type="number"
                                                   name="quantite[]"
                                                   ...
                                                   placeholder="14"
                                                   min="1"
                                                   value="<?php echo e(trim($quantites[$index] ?? '')); ?>"
                                                   required />
                                        </td>
                                        <td>
                                            <div class="rx-action-group">
                                                <?php if($index == 0): ?>
                                                    <button type="button" class="rx-btn-circle rx-btn-add" id="btnAddRow" title="Ajouter un médicament">
                                                        <i class="fas fa-plus"></i>
                                                    </button>
                                                <?php else: ?>
                                                    <button type="button" class="rx-btn-circle rx-btn-remove" onclick="removeRow(<?php echo e($seq); ?>)" title="Supprimer">
...
<div id="rxOverlay" class="rx-overlay" aria-hidden="true">
    <div class="rx-overlay-spinner"></div>
    <div class="text-white fw-semibold">Mise à jour en cours…</div>
</div>
...
$(document).ready(function () {
    /* ---------- STATE ---------- */
    let rowSeq       = <?php echo e($initialCount); ?>;     // last used seq
    let rowCount     = <?php echo e($initialCount); ?>;     // current number of rows
    let produitsData = [];
    let currentFocus = -1;
    let debounceTimer;
    ...
    function updateCount() {
        $('#badgeCount').text(rowCount);
        $('#footerCount').text(rowCount);
    }
    updateCount();

    /* ---------- RE‑INDEX ROW NUMBERS (after removal) ---------- */
    ...
    /* ---------- LOAD PRODUCTS AND INITIALIZE AUTOCOMPLETE ---------- */
    function initAutocompleteForAllRows() {
        $('.med-input').each(function () {
    ...
    function addRow() {
        ...
        <tr id="row_${seq}" data-seq="${seq}" style="--delay:${(rowCount-1)*50}ms;animation:rowFadeIn .35s ease both">
            ...
            <td>
                <div class="rx-action-group">
                    <button type="button" class="rx-btn-circle rx-btn-remove" onclick="removeRow(${seq})" title="Supprimer">
                        <i class="fas fa-trash-alt"></i>
                    </button>
                </div>
            </td>
    ...
    function initAC(seq) {
        ...
        $in.off('focus').on('focus', function () {
            const val = $(this).val().trim();
            if (val.length >= 2) showSuggestions(val, $in, $drop);
        });
        $in.off('blur').on('blur', function () {
    ...
    function showSuggestions(term, $in, $drop) {
        ...
        // attach dropdown to body for proper positioning
    ...
    /* ---------- HELPERS ---------- */
    function overlayShow() { $('#rxOverlay').addClass('active').attr('aria-hidden','false'); }
    function overlayHide() { $('#rxOverlay').removeClass('active').attr('aria-hidden','true'); }
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\JUSTO TCHEUMANI\DESKTOP\CMCU\Nouveaudossier\Proj\cmcu\cmcuapp\resources\views/admin/prescriptions/ordonance_edit.blade.php ENDPATH**/ ?>