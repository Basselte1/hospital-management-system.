...
<head>
    <meta charset="utf-8">
    <title>Reçu {{ $vente->numero_vente }}</title>
    <style>
        @page { margin: 20px; }
        body {
...
<body>
    ...
    <div class="info-section">
        ...
        <div class="info-row">
            ...
            <span class="info-value"><strong>{{ $vente->date_paiement->format('d/m/Y H:i') }}</strong></span>
        </div>
    </div>
    ...
    <div class="info-section">
        ...
        @if($vente->isPatientSale())
            <div class="info-row">
                ...
                <span class="info-value"><strong>{{ $vente->patient->name }} {{ $vente->patient->prenom }}</strong></span>
            </div>
            <div class="info-row">
                <span class="info-label">Dossier N°:</span>
                <span class="info-value">{{ $vente->patient->numero_dossier }}</span>
            </div>
            @if($vente->ordonance)
            <div class="info-row">
                <span class="info-label">Ordonnance:</span>
                <span class="info-value">{{ $vente->ordonance->created_at->format('d/m/Y') }}</span>
            </div>
            @endif
        @else
            <div class="info-row">
                <span class="info-label">Nom:</span>
                <span class="info-value"><strong>{{ $vente->client->nom ?? 'N/A' }} {{ $vente->client->prenom ?? '' }}</strong></span>
            </div>
            <div class="info-row">
                <span class="info-label">Institution:</span>
                <span class="info-value">{{ $vente->client->motif ?? '' }}</span>
            </div>
        @endif
    </div>
    ...
    <div class="total-box">
        ...
        <div style="font-size: 24pt; margin-top: 10px;">
            <strong>{{ number_format($vente->montant_paye, 0, ',', ' ') }} FCFA</strong>
        </div>
    </div>
    ...
    <div class="payment-info">
        ...
        <div class="info-row">
            ...
            <span class="info-value"><strong>{{ $vente->mode_paiement ?? 'N/A' }}</strong></span>
        </div>
        @if($vente->reference_paiement)
        <div class="info-row">
            <span class="info-label">Référence:</span>
            <span class="info-value">{{ $vente->reference_paiement }}</span>
        </div>
        @endif
        <div class="info-row">
            <span class="info-label">Caissier:</span>
            <span class="info-value">{{ $vente->caissier->name ?? 'N/A' }}</span>
        </div>
        <div class="info-row">
            <span class="info-label">Pharmacien:</span>
            <span class="info-value">{{ $vente->pharmacien->name ?? 'N/A' }}</span>
        </div>
    </div>
    ...
    <div style="margin-top: 50px; padding: 20px; border: 2px dashed #28a745; background-color: #f8f9fa;">
        <p style="margin: 0; text-align: center;">
            ...
            Ce reçu atteste du paiement intégral de la facture {{ $vente->numero_vente }}.<br>
