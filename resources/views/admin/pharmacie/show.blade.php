...
@section('title', 'CMCU | Détail Vente')
@section('breadcrumb', 'Pharmacie / Détail Vente')
@section('page_title', 'Détail Vente')
        ...
        <main class="tw-flex-1 tw-p-4 sm:tw-p-6 lg:tw-p-8">
            ...
            <div class="tw-mb-6 tw-flex tw-items-center tw-justify-between">
                <div>
                    ...
                    <p class="tw-text-sm tw-text-slate-500 tw-mt-1">{{ $vente->numero_vente }}</p>
                </div>
            ...
            <div class="tw-grid tw-grid-cols-1 lg:tw-grid-cols-3 tw-gap-6">
                    ...
                    <div class="tw-bg-white tw-rounded-2xl tw-shadow-sm tw-border tw-border-slate-100">
                        ...
                        <div class="tw-p-6 tw-grid tw-grid-cols-1 sm:tw-grid-cols-2 tw-gap-4">
                            <div>
                                ...
                                <p class="tw-text-base tw-font-semibold tw-text-[#1D4ED8] tw-mb-0">{{ $vente->numero_vente }}</p>
                            </div>
                            <div>
                                <p class="tw-text-xs tw-text-slate-400 tw-uppercase tw-tracking-wide tw-mb-1">Date</p>
                                <p class="tw-text-base tw-text-slate-700 tw-mb-0">{{ $vente->created_at->format('d/m/Y H:i') }}</p>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <p class="tw-text-base tw-text-slate-700 tw-mb-0">{{ $vente->pharmacien->name ?? 'N/A' }}</p>
                            </div>
                            <div class="sm:tw-col-span-2">
                                ...
                                @if($vente->isPatientSale())
                                    <p class="tw-text-base tw-text-slate-700 tw-mb-0">
                                        ...
                                        {{ $vente->patient->name }} {{ $vente->patient->prenom }}
                                        <span class="tw-text-slate-400 tw-text-sm tw-ml-2">Dossier N°: {{ $vente->patient->numero_dossier }}</span>
                                    </p>
                                    @if($vente->ordonance)
                                    <p class="tw-text-sm tw-text-[#14B8A6] tw-mt-1 tw-mb-0">
                                        <i class="fas fa-file-prescription tw-mr-1"></i> Ordonnance du {{ $vente->ordonance->created_at->format('d/m/Y') }}
                                    </p>
                                    @endif
                                @else
                                    <p class="tw-text-base tw-text-slate-700 tw-mb-0">
                                        <i class="fas fa-hospital tw-text-[#14B8A6] tw-mr-1"></i>
                                        {{ $vente->client->nom ?? 'N/A' }} {{ $vente->client->prenom ?? '' }}
                                    </p>
                                    @if($vente->client && $vente->client->motif)
                                    <p class="tw-text-sm tw-text-slate-400 tw-mt-0.5 tw-mb-0">{{ $vente->client->motif }}</p>
                                    @endif
                                @endif
                            </div>
                            @if($vente->notes)
                            <div class="sm:tw-col-span-2">
                                ...
                                <p class="tw-text-sm tw-text-slate-600 tw-mb-0">{{ $vente->notes }}</p>
                            </div>
                            @endif
                        </div>
                    </div>
                            ...
                            <table class="tw-w-full tw-text-sm">
                                ...
                                <tbody class="tw-divide-y tw-divide-slate-100">
                                    ...
                                    <tr>
                                        ...
                                        <td class="tw-px-4 tw-py-3 tw-text-center tw-text-slate-700">{{ $item->quantite }}</td>
                                        <td class="tw-px-4 tw-py-3 tw-text-right tw-text-slate-600">{{ number_format($item->prix_unitaire) }} FCFA</td>
                                        <td class="tw-px-4 tw-py-3 tw-text-right tw-font-semibold tw-text-slate-800">{{ number_format($item->montant_ligne) }} FCFA</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                                <tfoot>
                                    <tr class="tw-bg-slate-50 tw-border-t tw-border-slate-200">
                                        ...
                                        <td class="tw-px-4 tw-py-3 tw-text-right tw-font-bold tw-text-[#1D4ED8] tw-text-base">{{ number_format($vente->montant_total) }} FCFA</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
                ...
                <div class="tw-space-y-6">
                    ...
                    <div class="tw-bg-white tw-rounded-2xl tw-shadow-sm tw-border tw-border-slate-100 tw-overflow-hidden">
                        <div class="tw-px-6 tw-py-4 {{ $vente->isSoldee() ? 'tw-bg-green-500' : 'tw-bg-amber-400' }}">
                            <div class="tw-flex tw-items-center tw-gap-2">
                                <i class="fas {{ $vente->isSoldee() ? 'fa-check-circle' : 'fa-clock' }} tw-text-white"></i>
                        ...
                        <div class="tw-p-6">
                            @if($vente->isSoldee())
                                ...
                                <div class="tw-space-y-2 tw-text-sm">
                                    <p class="tw-mb-0"><span class="tw-text-slate-500">Montant Payé :</span> <span class="tw-font-semibold tw-text-slate-800">{{ number_format($vente->montant_paye) }} FCFA</span></p>
                                    <p class="tw-mb-0"><span class="tw-text-slate-500">Date Paiement :</span> <span class="tw-text-slate-800">{{ $vente->date_paiement->format('d/m/Y H:i') }}</span></p>
                                    <p class="tw-mb-0"><span class="tw-text-slate-500">Reçu par :</span> <span class="tw-text-slate-800">{{ $vente->caissier->name ?? 'N/A' }}</span></p>
                                    @if($vente->mode_paiement)
                                    <p class="tw-mb-0"><span class="tw-text-slate-500">Mode :</span> <span class="tw-text-slate-800">{{ $vente->mode_paiement }}</span></p>
                                    @endif
                                    @if($vente->reference_paiement)
                                    <p class="tw-mb-0"><span class="tw-text-slate-500">Réf. :</span> <span class="tw-text-slate-800">{{ $vente->reference_paiement }}</span></p>
                                    @endif
                                </div>
                            @else
                                ...
                                <p class="tw-text-2xl tw-font-bold tw-text-red-500 tw-mb-4">{{ number_format($vente->montant_total) }} FCFA</p>

                                @if(in_array(auth()->user()->role_id, [1, 6, 7]))
                                <hr class="tw-border-slate-100 tw-my-4">
                                <form action="{{ route('pharmacie.sales.soldee', $vente->id) }}" method="POST" class="tw-space-y-3">
                                    @csrf
                                    <div>
                    ...
                    <div class="tw-bg-white tw-rounded-2xl tw-shadow-sm tw-border tw-border-slate-100">
                        ...
                        <div class="tw-p-5 tw-space-y-2">
                            <a href="{{ route('pharmacie.sales.invoice', $vente->id) }}" target="_blank"
                            ...
                            @if($vente->isSoldee())
                            <a href="{{ route('pharmacie.sales.receipt', $vente->id) }}" target="_blank"
