...
@section('title', 'CMCU | Modifier Bon de Commande')
    ...
    <div id="content" class="tw-flex tw-flex-col tw-flex-1 tw-min-w-0 tw-overflow-y-auto">
        ...
        <main class="tw-flex-1 tw-p-4 sm:tw-p-6 lg:tw-p-8">
            ...
            <div class="tw-mb-6 tw-flex tw-items-center tw-justify-between tw-gap-3">
                <div>
                    <h1 class="tw-text-2xl tw-font-bold tw-text-slate-800">Modifier Bon de Commande</h1>
            ...
            <form action="{{ route('bon-commandes.update', $bonCommande->id) }}" method="POST" id="bonCommandeForm">
                @csrf
                @method('PUT')
                            ...
                            <div class="tw-grid tw-grid-cols-1 sm:tw-grid-cols-2 tw-gap-4">
                                <div>
                                    ...
                                    <input type="text" id="fournisseur_nom" name="fournisseur_nom"
                                           value="{{ old('fournisseur_nom', $bonCommande->fournisseur_nom) }}" required
                                ...
                                <div>
                                    ...
                                    <input type="email" id="fournisseur_email" name="fournisseur_email"
                                           value="{{ old('fournisseur_email', $bonCommande->fournisseur_email) }}"
                                ...
                                <div>
                                    ...
                                    <input type="text" id="fournisseur_telephone" name="fournisseur_telephone"
                                           value="{{ old('fournisseur_telephone', $bonCommande->fournisseur_telephone) }}"
                                ...
                                <div>
                                    ...
                                    <input type="text" id="fournisseur_adresse" name="fournisseur_adresse"
                                           value="{{ old('fournisseur_adresse', $bonCommande->fournisseur_adresse) }}"
                                           class="tw-w-full tw-rounded-xl tw-border tw-border-slate-200 tw-bg-slate-50 tw-px-3 tw-py-2 tw-text-sm focus:tw-outline-none focus:tw-ring-2 focus:tw-ring-[#BFDBFE]">
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="tw-grid tw-grid-cols-1 sm:tw-grid-cols-2 tw-gap-4">
                                <div>
                                    ...
                                    <input type="date" id="date_commande" name="date_commande" required
                                           value="{{ old('date_commande', $bonCommande->date_commande->format('Y-m-d')) }}"
                                ...
                                <div>
                                    ...
                                    <input type="date" id="date_livraison_souhaitee" name="date_livraison_souhaitee"
                                           value="{{ old('date_livraison_souhaitee', optional($bonCommande->date_livraison_souhaitee)->format('Y-m-d')) }}"
                                           class="tw-w-full tw-rounded-xl tw-border tw-border-slate-200 tw-bg-slate-50 tw-px-3 tw-py-2 tw-text-sm focus:tw-outline-none focus:tw-ring-2 focus:tw-ring-[#BFDBFE]">
                                </div>
                                <div class="sm:tw-col-span-2">
                                    <label for="notes" class="tw-block tw-text-xs tw-font-medium tw-text-slate-600 tw-mb-1.5">Notes / Remarques</label>
                                    <textarea id="notes" name="notes" rows="3"
                                              class="tw-w-full tw-rounded-xl tw-border tw-border-slate-200 tw-bg-slate-50 tw-px-3 tw-py-2 tw-text-sm focus:tw-outline-none focus:tw-ring-2 focus:tw-ring-[#BFDBFE] tw-resize-none">{{ old('notes', $bonCommande->notes) }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <table class="tw-w-full tw-text-sm" id="itemsTable">
                                ...
                                <tbody id="itemsBody" class="tw-divide-y tw-divide-slate-100">
                                    ...
                                    <tr class="hover:tw-bg-slate-50/50">
                                        <td class="tw-px-4 tw-py-3">
                                            <input type="hidden" name="items[{{ $index + 1 }}][produit_id]" class="produit-id" value="{{ $item->produit_id }}">
                                            <input type="text" name="items[{{ $index + 1 }}][designation]"
                                                   value="{{ $item->designation }}" required readonly
                                                   class="tw-w-full tw-rounded-lg tw-border tw-border-slate-200 tw-bg-slate-50 tw-px-3 tw-py-1.5 tw-text-sm focus:tw-outline-none focus:tw-ring-2 focus:tw-ring-[#BFDBFE] tw-cursor-pointer designation-input">
                                        </td>
                                        <td class="tw-px-4 tw-py-3">
                                            <select name="items[{{ $index + 1 }}][categorie]" required
                                                    class="tw-w-full tw-rounded-lg tw-border tw-border-slate-200 tw-bg-slate-50 tw-px-3 tw-py-1.5 tw-text-sm focus:tw-outline-none focus:tw-ring-2 focus:tw-ring-[#BFDBFE] categorie-select">
                                                <option value="PHARMACEUTIQUE" {{ $item->categorie === 'PHARMACEUTIQUE' ? 'selected' : '' }}>PHARMACEUTIQUE</option>
                                                <option value="MATERIEL"       {{ $item->categorie === 'MATERIEL'       ? 'selected' : '' }}>MATERIEL</option>
                                                <option value="ANESTHESISTE"   {{ $item->categorie === 'ANESTHESISTE'   ? 'selected' : '' }}>ANESTHESISTE</option>
                                            </select>
                                        </td>
                                        <td class="tw-px-4 tw-py-3">
                                            <input type="number" name="items[{{ $index + 1 }}][quantite]"
                                                   min="1" value="{{ $item->quantite_commandee }}" required
                                                   class="tw-w-full tw-rounded-lg tw-border tw-border-slate-200 tw-bg-slate-50 tw-px-3 tw-py-1.5 tw-text-sm focus:tw-outline-none focus:tw-ring-2 focus:tw-ring-[#BFDBFE] quantite-input">
                                        </td>
                                        <td class="tw-px-4 tw-py-3">
                                            <input type="number" name="items[{{ $index + 1 }}][prix_unitaire]"
                                                   min="0" value="{{ $item->prix_unitaire }}" required
                                                   class="tw-w-full tw-rounded-lg tw-border tw-border-slate-200 tw-bg-slate-50 tw-px-3 tw-py-1.5 tw-text-sm focus:tw-outline-none focus:tw-ring-2 focus:tw-ring-[#BFDBFE] prix-input">
                                        </td>
                                        <td class="tw-px-4 tw-py-3 tw-font-medium tw-text-slate-700 montant-cell">{{ number_format($item->montant_ligne, 0, ',', ' ') }} FCFA</td>
                                ...
                                <tfoot>
                                    <tr class="tw-bg-[#BFDBFE]/20 tw-border-t tw-border-[#BFDBFE]">
                                        ...
                                        <td colspan="2" class="tw-px-4 tw-py-3 tw-font-bold tw-text-[#1D4ED8]" id="totalAmount">{{ number_format($bonCommande->montant_total, 0, ',', ' ') }} FCFA</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
...
let itemCounter = {{ $bonCommande->items->count() }};
let currentRow = null;

$(document).ready(function() {

    $('#itemsBody tr').each(function() { calculateLineTotal($(this)); });
    calculateGrandTotal();
