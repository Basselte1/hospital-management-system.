...
<table width="100%" style="border-collapse:collapse; margin-bottom:8px;">
    <tr>
        <td width="15%" style="vertical-align:middle; padding-right:8px;">
            <img src="{{ public_path('admin/images/logo.jpg') }}" style="width:70px;height:auto;" alt="Logo">
        </td>
...
<table>
    ...
    <tbody>
        @forelse($tFactures as $f)
        <tr>
            <td style="text-align:center; font-weight:600;">{{ $f->numero }}</td>
            <td>{{ $f->name }}</td>
            <td>
                @if(!empty($f->examens))
                    <ul class="examen-list">
                        @foreach($f->examens as $ex)
                            <li>
                                {{ $ex['libelle'] ?? '—' }}
                                @if(!empty($ex['montant']))
                                    <span style="color:#555;">({{ number_format($ex['montant'],0,',',' ') }} F)</span>
                                @endif
                            </li>
                        @endforeach
                    </ul>
            ...
            <td style="text-align:right;">{{ number_format($f->montant,0,',',' ') }}</td>
            <td style="text-align:right;">{{ number_format($f->partAssurance,0,',',' ') }}</td>
            <td style="text-align:right;">{{ number_format($f->partPatient,0,',',' ') }}</td>
            <td style="text-align:right; color:#1e8449; font-weight:600;">
                {{ number_format($f->percu,0,',',' ') }}
            </td>
            <td style="text-align:right; color:{{ $f->reste > 0 ? '#c0392b' : '#1e8449' }}; font-weight:600;">
                {{ number_format($f->reste,0,',',' ') }}
            </td>
            <td style="text-align:center;">{{ $f->demarcheur ?? $f->medecin }}</td>
            <td>{{ $f->medecin }}</td>
            <td style="text-align:center;">
                @if(strtolower($f->statut) === 'soldée')
    ...
    <tfoot>
        <tr>
            ...
            <td style="text-align:right;">{{ number_format($totalPartAssurance,0,',',' ') }} F</td>
            <td style="text-align:right;">{{ number_format($totalPartPatient,0,',',' ') }} F</td>
            <td style="text-align:right;">{{ number_format($totalPercu,0,',',' ') }} F</td>
            <td style="text-align:right;">{{ number_format($totalReste,0,',',' ') }} F</td>
            <td colspan="3"></td>
        </tr>
    </tfoot>
</table>
        ...
        <tbody>
            ...
            <tr style="border-top:1px solid #1a5276;">
                ...
                <td style="padding:3px 12px; border:none; font-weight:700; color:#1a5276; text-align:right;">
                    {{ number_format($totalPercu, 0, ',', ' ') }} FCFA
                </td>
            </tr>
        </tbody>
    </table>
