    ...
    <table width="100%" style="border-collapse: collapse; margin-bottom: 20px;">
        <tr>
            <td style="vertical-align: top;">
                ...
                <p><small>{{ $user->specialite }}</small></p>
                <p><small>ONMC: {{ $user->onmc }}</small></p>
            </td>
            <td style="vertical-align: top; text-align: right;">
                <p><small>{{ \Carbon\Carbon::parse($ordonance->created_at)->format('d/m/Y H:i') }}</small></p>
                <p><strong>Patient :</strong> {{ $patient->name }} {{ $patient->prenom }}</p>
            </td>
        </tr>
    </table>
        ...
        <tbody>
            @for ($i = 0; $i < $rowCount; $i++)
                <tr>
                    ...
                    <td style="text-align:center;">{{ $quantites[$i] ?? '' }}</td>
                </tr>
            @endfor
        </tbody>
    </table>
