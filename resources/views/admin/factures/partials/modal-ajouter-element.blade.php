{{--
    ============================================================
    PARTIAL : modal-ajouter-element.blade.php
    À inclure dans la vue des factures de consultation :
        @include('admin.factures.partials.modal-ajouter-element')

    Prérequis : Bootstrap 5 + Font Awesome chargés dans le layout
    ============================================================
--}}
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                {{-- Alertes session (re-ouverture de la modal si erreur) --}}
                @if(session('success'))
                ...
                <form action="{{ route('factures.ajouter-element') }}" method="POST"
                    ...
                    {{-- Champs cachés injectés par le JS --}}
                    ...
                    <div class="mb-4">
                        ...
                        {{-- Champ caché qui contiendra la valeur sélectionnée --}}
                    ...
                    {{-- ② Sous-formulaire COMMUN (partagé examen + soin) ─── --}}
                    <div id="form-details" class="d-none border rounded p-3 bg-light">
                        ...
                        {{-- Sélecteur "acte existant" (chargé via AJAX) --}}
                        <div class="mb-3">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-3">
                                ...
                                <input type="number" class="form-control @error('montant_unitaire') is-invalid @enderror"
                                       name="montant_unitaire" id="input-montant-unitaire"
                                       value="{{ old('montant_unitaire', 0) }}"
                                       min="0" step="100" placeholder="5000">
                                @error('montant_unitaire')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            {{-- Champ infirmière — visible uniquement pour soin_infirmier --}}
                            <div class="col-md-6" id="row-infirmiere" style="display:none">
                                ...
                                <input type="text" class="form-control"
                                       ...
                                       value="{{ old('infirmiere') }}"
                                       placeholder="Nom de l'infirmière"
                                       maxlength="150">
                            </div>
                        </div>

                        {{-- Récapitulatif total calculé en temps réel --}}
...
{{-- ================================================================
     SCRIPT — scope isolé dans un IIFE pour éviter les conflits
     ================================================================ --}}
@push('scripts')
<script>
(function () {
    ...
    function chargerActes(type) {
        ...
        const url = type === 'examen_labo'
            ? `{{ route('api.factures.patient-examens') }}?patient_id=${patientId}`
            : `{{ route('api.factures.patient-soins') }}?patient_id=${patientId}`;
