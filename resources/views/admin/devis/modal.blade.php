            ...
            <div class="modal-body tw-p-6">
                <form id="devis_form" action="{{ route('devis.store') }}" method="POST">
                    ...
                    <ul class="nav nav-tabs tw-border-b tw-border-slate-200 tw-mb-4 tw-gap-1" id="devisTabs" role="tablist">
                        ...
                        ] as [$target, $id, $icon, $label, $badge, $badgeCls, $active])
                        <li class="nav-item" role="presentation">
                            <button class="nav-link tw-flex tw-items-center tw-gap-2 tw-text-sm tw-px-4 tw-py-2.5 tw-rounded-t-xl tw-border-0 {{ $active ? 'active' : '' }}"
                                id="{{ $id }}" data-bs-toggle="tab" data-bs-target="#{{ $target }}" type="button" role="tab">
                                <i class="fas {{ $icon }}"></i> {{ $label }}
                                @if($badge)
                                <span class="tw-inline-flex tw-rounded-full {{ $badgeCls }} tw-text-xs tw-font-medium tw-px-2 tw-py-0.5" id="{{ $badge }}">0</span>
                                @endif
                            </button>
                        </li>
                        @endforeach
                    ...
                    <div class="tab-content" id="devisTabsContent">
                        ...
                        <div class="tab-pane fade" id="hospitalization" role="tabpanel">
                            <div class="tw-rounded-xl tw-border tw-border-slate-200 tw-overflow-hidden tw-mb-3">
                                ...
                                ] as [$num, $type, $qtyName, $puName, $totalId, $defaultPu])
                                <div class="tw-grid tw-grid-cols-12 tw-divide-x tw-divide-slate-100 tw-border-b tw-border-slate-100">
                                    <div class="tw-col-span-1 tw-flex tw-items-center tw-justify-center tw-px-3 tw-py-2.5 tw-text-xs tw-text-slate-400">{{ $num }}</div>
                                    <div class="tw-col-span-5 tw-flex tw-items-center tw-px-3 tw-py-2.5 tw-text-sm tw-text-slate-600">{{ $type }}</div>
                                    <div class="tw-col-span-2 tw-px-3 tw-py-2.5">
                                        <input type="number" name="{{ $qtyName }}" id="{{ $qtyName }}" value="0" min="0"
                                    ...
                                    <div class="tw-col-span-2 tw-px-3 tw-py-2.5">
                                        <input type="number" name="{{ $puName }}" id="{{ $puName }}" value="{{ $defaultPu }}" min="0"
                                            class="tw-w-full tw-rounded-lg tw-border tw-border-slate-200 tw-bg-slate-50 tw-px-2 tw-py-1.5 tw-text-sm focus:tw-outline-none focus:tw-ring-1 focus:tw-ring-[#BFDBFE]">
                                    </div>
                                    <div class="tw-col-span-2 tw-px-3 tw-py-2.5">
                                        <input type="text" id="{{ $totalId }}" value="0" readonly
