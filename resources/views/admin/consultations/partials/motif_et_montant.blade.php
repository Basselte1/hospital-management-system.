{{--
    Partial: motif_et_montant.blade.php
    Embedded inside a <table> as a <tbody> — preserves the host structure.
--}}
<tbody id="editMotifMontform" style="display: none;">
    <form action="{{ route('patients.motif_montant.update', $patient->id) }}" method="POST">
        ...
        {{-- Nom & prénom --}}
        <tr>
            <td class="tw-px-3 tw-py-2.5 tw-align-middle {{ $labelCls }}">
                Nom & prénom
            </td>
            <td class="tw-px-3 tw-py-2.5">
                <div class="tw-flex tw-gap-2">
                    <input name="name"   class="{{ $inputCls }}" value="{{ old('name',   $patient->name)   }}" type="text" placeholder="Nom">
                    <input name="prenom" class="{{ $inputCls }}" value="{{ old('prenom', $patient->prenom) }}" type="text" placeholder="Prénom">
                </div>
            </td>
        </tr>

        {{-- Médecin / Infirmier (récupère le médecin si présent, sinon l’infirmier) --}}
        <tr class="tw-bg-slate-50">
            ...
            <td class="tw-px-3 tw-py-2.5">
                ...
                <select name="medecin_r" id="medecin_r" required class="{{ $inputCls }}">
                    <option value="">— Sélectionner médecin / infirmier —</option>
                    @foreach ($medecin as $user)
                        <option value="{{ $user->name }} {{ $user->prenom }}"
                            {{ $medecinSelectedText === ($user->name.' '.$user->prenom) ? 'selected' : '' }}>
        ...
        <tr>
            ...
            <td class="tw-px-3 tw-py-2.5">
                <select name="motif" id="motif" class="{{ $inputCls }}" onchange="new_ckChange(this)">
        ...
        <tr class="tw-bg-slate-50">
            <td class="tw-px-3 tw-py-2.5 tw-align-middle">
                <label for="details_motif" id="label_details_motif" class="{{ $labelCls }}">
                    Détails motif <span class="tw-text-red-500">*</span>
                </label>
            </td>
            <td class="tw-px-3 tw-py-2.5">
                <input name="details_motif" id="details_motif" class="{{ $inputCls }}"
                       value="{{ old('details_motif', $patient->details_motif) }}"
        ...
        <tr>
            ...
            <td class="tw-px-3 tw-py-2.5">
                <input name="montant" class="{{ $inputCls }} tw-w-48"
                       value="{{ old('montant', $patient->montant) }}"
        ...
        {{-- Assurance --}}
        <tr class="tw-bg-slate-50">
            <td class="tw-px-3 tw-py-2.5 tw-align-middle {{ $labelCls }}">Assurance</td>
            <td class="tw-px-3 tw-py-2.5">
                <input name="assurance" class="{{ $inputCls }}"
                       value="{{ old('assurance', $patient->assurance) }}"
        ...
        <tr>
            ...
            <td class="tw-px-3 tw-py-2.5">
                <input name="avance" class="{{ $inputCls }} tw-w-48"
                       value="{{ old('avance', $patient->avance) }}"
                       type="number" placeholder="0">
            </td>
        </tr>
        ...
        {{-- N° assurance --}}
        <tr class="tw-bg-slate-50">
            <td class="tw-px-3 tw-py-2.5 tw-align-middle {{ $labelCls }}">N° assurance</td>
            <td class="tw-px-3 tw-py-2.5">
                <input name="numero_assurance" class="{{ $inputCls }}"
        ...
        <tr>
            <td class="tw-px-3 tw-py-2.5 tw-align-middle {{ $labelCls }}">Taux prise en charge</td>
            <td class="tw-px-3 tw-py-2.5">
                <div class="tw-flex tw-items-center tw-gap-2">
                    <select name="prise_en_charge" id="prise_en_charge" required class="{{ $inputCls }} tw-w-28">
        ...
        {{-- Mode paiement --}}
        <tr class="tw-bg-slate-50">
            <td class="tw-px-3 tw-py-2.5 tw-align-middle {{ $labelCls }}">Moyen de paiement</td>
            <td class="tw-px-3 tw-py-2.5">
                <select name="mode_paiement" id="mode_paiement" class="{{ $inputCls }}">
                    <optgroup label="Monnaie électronique">
                        <option value="orange money"      {{ old('mode_paiement', $patient->mode_paiement) == 'orange money' ? 'selected' : '' }}>Orange Money</option>
                        <option value="mtn mobile money"  {{ old('mode_paiement', $patient->mode_paiement) == 'mtn mobile money' ? 'selected' : '' }}>MTN Mobile Money</option>
                    </optgroup>
                    <optgroup label="Autres moyens">
                        <option value="espèce"              {{ old('mode_paiement', $patient->mode_paiement) == 'espèce' ? 'selected' : '' }}>Espèce</option>
                        <option value="chèque"              {{ old('mode_paiement', $patient->mode_paiement) == 'chèque' ? 'selected' : '' }}>Chèque</option>
                        <option value="virement"            {{ old('mode_paiement', $patient->mode_paiement) == 'virement' ? 'selected' : '' }}>Virement</option>
                        <option value="bon de prise en charge" {{ old('mode_paiement', $patient->mode_paiement) == 'bon de prise en charge' ? 'selected' : '' }}>Bon de prise en charge</option>
                        <option value="autre"               {{ old('mode_paiement', $patient->mode_paiement) == 'autre' ? 'selected' : '' }}>Autre</option>
                    </optgroup>
                </select>
            </td>
        </tr>
